...
function RandomBauCua () {
	IMG.forEach(function(userItem,index) {
		...
		userItem.src = "assets/imgs/"+XiNgau[indexRD];
...
ChooseBC.forEach ((item,index) => {
	item.onclick = function () {
		if (ChipNow <= Balance) {
			...
			Balance-= ChipNow;
			item.insertAdjacentHTML ("beforeend","<img class='chipBet' style='--randomIndexT:"+coordT+"px; --randomIndexL:"+coordL+"px;' src='assets/imgs/Chip"+ChipNow+".png'>")
